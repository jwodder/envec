...
sub parseDetails($) {
 ...
 if ($doc->getElementById("${pre}nameRow")) { scrapeSection($doc, $pre) }
  ...
  my @parts;
  for my $i (0..10) {
   my $prefix = sprintf '%sctl%02d_', $pre, $i;
   push @parts, scrapeSection($doc, $prefix)
    if $doc->getElementById("${prefix}nameRow");
  }
  if (@parts == 2) { return joinCards NORMAL_CARD, $parts[0], $parts[1] }
  else {
   croak 'EnVec::Details::parseDetails: multipart details page has too ',
    @parts > 2 ? 'many' : 'few', ' parts'
  }
 }
}
